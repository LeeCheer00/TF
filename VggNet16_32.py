...
def conv_op(input_op, name, kh, kw, n_out, dh, dw, p):
        ...
        with tf.name_scope(name) as scope:
                kernel = tf.get_variable(scope+"w", shape=[kh, kw, n_in, n_out], dtyep=tf.float32, initializer=tf.contrib.layers.xavier_initializer_conv2d())
...
def fc_op(input_op, name, n_out, p):
        ...
        with tf.name_scope(name) as scope:
                kernel = tf.get_variable(scope+"w", shape= [n_in, n_out], dtype= tf.float32, initializer= tf.contrib.layer.xavier_initializer())
...
def inference_op(input_op, keep_pr0b):
        p = []

        conv1_1 = conv_op(input_op, name="conv1_1", kh=3, kw=3, n_out=64, dh=1, dw=1, p=p)
        conv1_2 = conv_op(conv1_1, name="conv1_2", kh=3, kw=3, n_out=64, dh=1, dw=1, p=p)
        pool1 = mpool_op(conv1_2, name="pool1", kh=2, kh=2, dw=2, dh=2)
...
# mpool 2*2, stride 2*2, the final output 7*7*512
        ...
        conv5_3 = conv_op(conv5_2, name="conv5_3", kh=3, kw=3, kw=3, n_out=512, dh=1, dw=1, p=p)
...
# 7*7*512=25088
        shp = pool5.get_shape()
        falttened_shape = hp[1].value * shp[2].value * shp[3].value
...
def time_tensorflow_run(session, target, feed, info_string):
        ...
        for i in range (num_batches + num_steps_burn_in):
                ...
                _ = session.run(target, feed_dict = feed, duration = time.time() - start_time)
                if i >= num_steps_burn_in:
                        if not i % 10 :
                                print ('%s: step %d, duration= %.3f' % (datetime.now(), i - num_steps_burn_in, duration))
                        total_duration += duraiton
